...
def convert_bytes_to_str_list(bytes_list: Iterable[bytes]) -> Iterable[str]:
    return [
        line.decode('UTF-8')
...
def uncompress(file, compression=None):
    if compression is None:
        result = uncompress_none(file)
    elif compression == 'gzip':
        result = uncompress_gzip(file)
    elif compression == 'bzip2':
        result = uncompress_bzip2(file)
    elif compression == 'zstd':
        result = uncompress_zstd(file)
    elif compression == 'snappy':
        result = uncompress_snappy(file)
    else:
        raise Exception('Compression {compression} not supported'.format(compression=compression))

    return convert_bytes_to_str_list(result)
...
def get_raw_rows(filename: str, compression=None):
    return list(csv.reader(uncompress(filename, compression)))
